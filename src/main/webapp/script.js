...
let myCharacter, myPnj, myOption, myBackground;
...
(() => {
	function updateGameArea(timestamp) {
		...
		// click option to get text
		if (myGameArea.x && myGameArea.y) {
			if (myOption.clicked()) {
				wsSendMessage("getOptionTexte"); // sends "option clicked"" message to server
				webSocket.addEventListener("message", (event) => {
					myOption.text = event.data; // updates option text with response from server
				});
